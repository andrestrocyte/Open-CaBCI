...
class Calcium():
    ...
    def binarize_onphase2(self,
        ...
        for k in trange(traces.shape[0], desc='binarizing continuous traces '+text):
            ...
            if text=='filtered fluorescence upphase':
                plt.plot(np.arange(temp.shape[0])/30.,temp+0.1)

                plt.plot(np.arange(temp.shape[0])/30, np.ones(temp.shape[0])*self.thresholds[k], 'r--')
                plt.title(text)
            ...
            # fill the data with 1s and also can pad here if desired
            buffer = 0
            starts_ends = []
            for p in range(xys.shape[0]):
                traces_bin[k,xys[p,0]:xys[p,1]+buffer] = 1
                starts_ends.append([xys[p,0],xys[p,1]+buffer])

            starts_ends = np.array(starts_ends)
            #
            self.starts_ends.append(starts_ends)
